...
import com.lam.tms.util.VerifyCodeUtils;
import com.lam.tms.vo.LoginUser;
import com.lam.tms.vo.TokenValue;
...
public class AuthController {
    ...
    public JsonResult<TokenValue> login(@RequestBody @Validated LoginUser user, BindingResult br) {
        ...
        if (!StringUtils.hasText(codeText) || !codeText.toLowerCase().equals(user.getCodeText().toLowerCase())) {
    ...
    public JsonResult<TokenValue> refresh(@RequestHeader(value = "${jwt.tokenHeader}") String completeToken) {
        ...
        if (!StringUtils.hasText(username)) {
            ...
            if (details != null && !StringUtils.hasText(details.getUsername())) {
                // 刷新 token 的值
